(function () {
  ...
  function renderMediation(d) {
    qs('#mediationTable').innerHTML = d.mediation.map(function (row) {
      return '<tr><td>' + tr(row.x) + ' → ' + tr(row.mediator) + '</td><td>' + fmt(row.indirect, 3) + '</td><td>[' + fmt(row.ci_low, 3) + ', ' + fmt(row.ci_high, 3) + ']</td><td>' + row.bootstrap_valid + '/' + row.bootstrap_iterations + '</td></tr>';
    }).join('');
    makeChart('mediationChart', Object.assign(baseOption(), {
      grid: { left: 156, right: 38, top: 24, bottom: 34, containLabel: true },
      yAxis: { type: 'category', data: d.mediation.map(function (x) { return tr(x.x) + ' → ' + tr(x.mediator); }).reverse(), axisLabel: { color: T.muted, fontSize: 11 }, axisLine: { lineStyle: { color: T.line } } },
      xAxis: { type: 'value', name: tr('间接关联'), splitLine: { lineStyle: { color: T.grid } }, axisLabel: { color: T.muted } },
      series: [{ type: 'scatter', symbolSize: 10, itemStyle: { color: T.secondary }, data: d.mediation.map(function (x) { return [x.indirect, tr(x.x) + ' → ' + tr(x.mediator)]; }).reverse(), markLine: { symbol: 'none', lineStyle: { color: T.earth, type: 'dashed' }, data: [{ xAxis: 0 }] } }]
    }));
  }
  ...
  function renderMl(d) {
    var rows = d.ml.filter(function (x) { return x.feature_set !== 'controls'; });
  ...
  function render(d) { renderMetricCards(d); renderAudit(d); renderDirect(d); renderMediation(d); renderHeterogeneity(d); renderMl(d); }
  function init() {
    qs('#langSwitch').addEventListener('click', function () { setLang(state.lang === 'zh' ? 'en' : 'zh'); });
    fetch('static/data/report.json').then(function (r) { return r.json(); }).then(function (d) { state.data = d; setLang(state.lang); });
